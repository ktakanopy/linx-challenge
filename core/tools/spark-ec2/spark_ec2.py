...
def parse_args():
    ...
    parser.add_option("--main-ami",
        help="Amazon Machine Image ID to use for the Main")
    parser.add_option(
    ...
    parser.add_option(
        "--main-spot", action="store_true", default=False,
        help="If specified, launch main as spot instance using the same " +
             "bid and instance type of the subordinate ones")
    parser.add_option(
...
def launch_cluster(conn, opts, cluster_name):
    ...
    if opts.security_group_prefix is None:
        ...
        main_group = get_or_make_group(conn, opts.security_group_prefix + "-main", opts.vpc_id)
        subordinate_group = get_or_make_group(conn, opts.security_group_prefix + "-subordinates", opts.vpc_id)
    ...
    if opts.main_ami is None:
        opts.main_ami = get_spark_ami(opts.main_instance_type, opts.region, opts.spark_ec2_git_repo, opts.spark_ec2_git_branch)
    ...
    try:
        main_image = conn.get_all_images(image_ids=[opts.main_ami])[0]
    except:
        print >> stderr, "Could not find AMI " + opts.main_ami
        sys.exit(1)
    ...
    if existing_mains:
        ...
        if opts.main_spot:
            ...
            main_req = conn.request_spot_instances(
                ...
                user_data=user_data_content,
                instance_profile_name=opts.instance_profile_name)
            my_main_req_id = [req.id for req in main_req]

            # TODO: refactor duplicated spot waiting code
            start_time = datetime.now()
            print("Waiting for main spot instance to be granted... Request ID: %s " % my_main_req_id)
            try:
                while True:
                    time.sleep(10)
                    reqs = conn.get_all_spot_instance_requests(my_main_req_id)
                    active_instance_ids = filter(lambda req: req.state == "active", reqs)
                    invalid_states = ["capacity-not-available", "capacity-oversubscribed", "price-too-low"]
                    invalid = filter(lambda req: req.status.code in invalid_states, reqs)
                    if len(invalid) > 0:
                    ...
                    if len(active_instance_ids) == 1:
                        print("Main spot instance granted")
                        main_res = conn.get_all_reservations([r.instance_id for r in active_instance_ids])
                        main_nodes = main_res[0].instances
                        break
                    else:
                        print("Main spot instance not granted yet, waiting longer")

                    if (datetime.now() - start_time).seconds > opts.spot_timeout * 60:
                        raise Exception("Timed out while waiting for main spot instance")
            except:
                print("Error: %s" % sys.exc_info()[1])
                print("Canceling main spot instance requests")
                conn.cancel_spot_instance_requests(my_main_req_id)
                # Log a warning if any of these requests actually launched instances:
                (main_nodes, subordinate_nodes) = get_existing_cluster(
                    conn, opts, cluster_name, die_on_error=False)
                running = len(main_nodes) + len(subordinate_nodes)
